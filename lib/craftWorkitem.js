...
`;
  ...
  const {appDir, serviceFilePath} = getPathsForNewService(`${name}-workitem`);
  ...
  const {goblinFilePath} = getPathsForNewService('service', workitemFolderPath);
  tryGenerateFile(goblinFilePath, workitemServiceFile);
  const uiPath = getPathsForNewService('ui', workitemFolderPath);
