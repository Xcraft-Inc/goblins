const {tryGenerateFile, preparePathsForNewService} = require('./helpers.js');

const writeQueue = (name) => {
  const {appDir, serviceFilePath} = preparePathsForNewService(name);
...
const writeWorker = (name) => {
  ...
  const {appDir, serviceFilePath} = preparePathsForNewService(workerName);
  const serviceFile = `
const Goblin = require('xcraft-core-goblin');
exports.xcraftCommands = function () {
